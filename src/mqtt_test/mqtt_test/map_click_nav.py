"""
map_click_nav.py — Map-only click-to-navigate for mapuse28

Single window:
  "Map Nav - mapuse28" — mapuse28.pgm with overlays:
      ● Green circle + arrow  = live robot position (/odom)
      ● Red   circle + cross  = last sent navigation goal

LEFT CLICK on the map to send a goal_pose. The pixel coordinate is
converted to map-frame metres using the mapuse28.yaml origin and
resolution, then published as a PoseStamped on /goal_pose.
...
class MapClickNavNode(Node):
    """ROS2 node that publishes clicked map positions as navigation goals."""

    def __init__(self):
        super().__init__("map_click_nav")
        ...
        self._topic = self.get_parameter("mqtt_topic").get_parameter_value().string_value
        username = self.get_parameter("mqtt_username").get_parameter_value().string_value
        password = self.get_parameter("mqtt_password").get_parameter_value().string_value
        
        self._pub = self.create_publisher(PoseStamped, "/goal_pose", 10)
        self.xy_pub = self.create_publisher(String, '/goal_xy', 10)
        ...
        self.get_logger().info("MapClickNav ready — click the map to navigate")
    ...
    def send_goal(self, x: float, y: float) -> None:
        """Publish a PoseStamped goal on /goal_pose."""
        msg = PoseStamped()
        ...
        payload_json = json.dumps({"x": x, "y": y})
        msg_str = String()
        msg_str.data = payload_json
        self.xy_pub.publish(msg_str)
        self.get_logger().info(f"Published to /goal_xy: {payload_json}")
        ...
        if self._mqtt_connected:
            ...
            try:
                self._mqtt_client.publish(self._topic, json.dumps(mqtt_payload))
...
def main(args=None):
    ...
    def on_map_mouse(event, col, row, flags, param):
        ...
        node.get_logger().info(
            f"Click ({col},{row}) → map ({x_m:.3f}, {y_m:.3f}) m"
        )
        node.send_goal(x_m, y_m)

    cv2.namedWindow("Map Nav - mapuse28")
    cv2.setMouseCallback("Map Nav - mapuse28", on_map_mouse)
    ...
    while rclpy.ok():
        ...
        cv2.putText(
            m, "Robot", (lx + 15, ly + 4),
            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 210, 0), 1,
        )
